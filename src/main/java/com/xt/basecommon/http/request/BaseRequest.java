...
import com.xt.basecommon.http.XTHttp;
import com.xt.basecommon.http.api.ApiService;
...
import com.xt.basecommon.http.interceptor.BaseDynamicInterceptor;
import com.xt.basecommon.http.interceptor.CacheInterceptor;
import com.xt.basecommon.http.interceptor.CacheInterceptorOffline;
import com.xt.basecommon.http.interceptor.HeadersInterceptor;
import com.xt.basecommon.http.interceptor.NoCacheInterceptor;
...
public abstract class BaseRequest<R extends BaseRequest> {
    ...
    protected HttpsUtil.SSLParams sslParams;
    ...
    public BaseRequest(String url) {
        this.url = url;
        context = XTHttp.getContext();
        XTHttp config = XTHttp.getInstance();
    ...
    public R cacheTime(long cacheTime) {
        if (cacheTime <= -1) {
            cacheTime = XTHttp.DEFAULT_CACHE_NEVER_EXPIRE;
        }
    ...
    public R certificates(InputStream... certificates) {
        this.sslParams = HttpsUtil.getSslSocketFactory(null, null, certificates);
        return (R) this;
    }
    ...
    public R certificates(InputStream bksFile, String password, InputStream... certificates) {
        this.sslParams = HttpsUtil.getSslSocketFactory(bksFile, password, certificates);
        return (R) this;
    }
    ...
    public void removeCache(String key) {
        XTHttp.getRxCache().remove(key).compose(RxUtil.<Boolean>io_main())
    ...
    private OkHttpClient.Builder generateOkClient() {
        if (readTimeOut <= 0 && writeTimeOut <= 0 && connectTimeout <= 0 && sslParams == null
                && cookies.size() == 0 && hostnameVerifier == null && proxy == null &&
                headers.isEmpty()) {
            OkHttpClient.Builder builder = XTHttp.getOkHttpClientBuilder();
            ...
            final OkHttpClient.Builder newClientBuilder = XTHttp.getOkHttpClient().newBuilder();
            if (readTimeOut > 0) {
            ...
            if (cookies.size() > 0) {
                XTHttp.getCookieJar().addCookies(cookies);
            }
    ...
    private Retrofit.Builder generateRetrofit() {
        if (converterFactories.isEmpty() && adapterFactories.isEmpty()) {
            return XTHttp.getRetrofitBuilder().baseUrl(baseUrl);
        } else {
            ...
            if (!converterFactories.isEmpty()) {
                ...
                List<Converter.Factory> listConverterFactory =
                        XTHttp.getRetrofit().converterFactories();
            ...
            if (!adapterFactories.isEmpty()) {
                ...
                List<CallAdapter.Factory> listAdapterFactory =
                        XTHttp.getRetrofit().callAdapterFactories();
    ...
    private RxCache.Builder generateRxCache() {
        final RxCache.Builder rxCacheBuilder = XTHttp.getRxCacheBuilder();
        switch (cacheMode) {
            case NO_CACHE://不使用缓存
            ...
            case DEFAULT://使用Okhttp的缓存
                if (this.cache == null) {
                    File cacheDirectory = XTHttp.getCacheDirectory();
                    if (cacheDirectory == null) {
                        cacheDirectory = new File(XTHttp.getContext().getCacheDir(), "okhttp-cache");
                    } else {
                        if (cacheDirectory.isDirectory() && !cacheDirectory.exists()) {
                            cacheDirectory.mkdirs();
                        }
                    }
                    this.cache = new Cache(cacheDirectory, Math.max(5 * 1024 * 1024,
                            XTHttp.getCacheMaxSize()));
                }
                ...
                final CacheInterceptor REWRITE_CACHE_CONTROL_INTERCEPTOR =
                        new CacheInterceptor(XTHttp.getContext(), cacheControlValue);
                final CacheInterceptorOffline REWRITE_CACHE_CONTROL_INTERCEPTOR_OFFLINE =
                        new CacheInterceptorOffline(XTHttp.getContext(), cacheControlValue);
            ...
            case ONLYCACHE:
            case CACHEANDREMOTE:
            case CACHEANDREMOTEDISTINGCT:
                interceptors.add(new NoCacheInterceptor());
                if (diskConverter == null) {
                    final RxCache.Builder tempRxCacheBuilder = rxCacheBuilder;
                    tempRxCacheBuilder.cachekey(Utils.checkNotNull(cacheKey,
                            "cacheKey == null"))
                            .cacheTime(cacheTime);
                    return tempRxCacheBuilder;
                } else {
                    final RxCache.Builder cacheBuilder = XTHttp.getRxCache().newBuilder();
